...
.addRow(
  ...
  .addPanel(
    graphPanel.new(
      'Request Throughputs',
      description='Coordinator level read and write count - Sum of each 1m rate for each operation type and per cluster',
      format='rps',
    ...
    .addTarget(
      prometheus.target(
        ...
        legendFormat='Sum of {{scope}} Request Rate for cluster {{cluster}}',
      )
    )
  )
  .addPanel(
    graphPanel.new(
      'Error throughputs',
      description='Sum of Timeouts, Failures, Unavailable Rates for each cluster',
      format='rps',
      datasource='Prometheus',
      transparent=true,
      fill=0,
...
.addRow(
 ...
 .addPanel(
  ...
  .addPanel(
    ...
    .addTarget(
      prometheus.target(
        'sum by (environment, cluster) (-1 * (rate(node_network_transmit_bytes_total{environment="$environment", cluster="$cluster", datacenter=~"$datacenter", rack=~"$rack", node=~"$node"}[1m])))',
